...
window.addEventListener('load', function () {
  document.querySelector('#subscribeme').addEventListener('click', function () {
    console.log(document.querySelector('#username').value);

    pushSubscribe(document.querySelector('#username').value)
  })
  document.querySelector('#unsubscribeme').addEventListener('click', function () {
    pushUnsubscribe()
  })
...
function pushSubscribe (name) {
  if ('showNotification' in ServiceWorkerRegistration.prototype) {
  console.log('showNotification',name)
      ...
      swRegistration.pushManager.subscribe({
        ...
        .then(function (pushSubscription) {
          ...
          return fetch('http://localhost:3000/webpush', {
            method: 'POST',
            body: JSON.stringify({action: 'subscribe', subscription: pushSubscription, user: name}),
